...
- **Excel ブック** … **ロググループ × ログストリーム**ごとにシートを分割。
  **各ロググループの最新のログストリーム以外のシートは既定で非表示**なので、
  シート見出しが増えすぎず、まず見たい直近のログにすぐ辿り着けます
  （非表示のシートはシート見出しの右クリック →「再表示」で表示できます）。
  フォントは **Meiryo UI**。JBoss EAP のログは区分（**デプロイ / 起動 / 停止 /
  エラー / 警告**）ごとに色分けし、**「主要イベント」シート**でアプリケーションの
  配備とサーバー本体の起動・停止をロググループ横断・時刻順に一覧できます。
...
# 複数ロググループを Excel 出力（ロググループ × ストリームごとにシート分割・画面にも表示）
...
**ロググループ × ログストリーム**ごとにシートを分けた Excel ファイルを作成します。
...
| シート | 内容 |
|---|---|
| `サマリ`（1 枚目） | 出力日時 / 対象期間 / **期間の判定基準** / AWS アカウント・プロファイル・リージョン / 抽出条件 / 取得上限 / 生ログの出力先 / シート構成、およびロググループ一覧（No・ロググループ・シート数・取得・期間外・出力・**エラー・警告・デプロイ・アンデプロイ・起動・停止の件数**・最古/最新日時・生ログ数） |
| `主要イベント`（2 枚目） | **アプリケーションのデプロイ**と **JBoss EAP の起動・停止**だけを、ロググループ横断で時刻順に並べた一覧 |
| `ストリーム一覧`（3 枚目） | **シートとログストリームの対応表**（No・ロググループ・ログストリーム・シート名・**状態（表示/非表示）**・出力件数・区分ごとの件数・最古/最新日時）。どのシートを「再表示」すればよいかはここで分かります |
| ロググループ × ログストリームごと | 1 行目にロググループ名、2 行目にログストリーム名、3 行目に期間・件数・抽出条件、4 行目が見出し、5 行目以降がログ |

各ログシートの列は次のとおりです（ログストリームはシートごとに固定のため、
列ではなく 2 行目に記載します）。
...
| D | メッセージ（セル内で折り返し表示。複数行ログはセル内改行として保持） |
| E | イベント時刻(JST) — CloudWatch がイベントを受け取った時刻。Excel の日時型 |
| F | timestamp(ms) — 数値 |
...
### 最新ストリーム以外のシートは既定で非表示

1 つのロググループには、サーバー 1 台ごと・日付ごとなど複数のログストリームが
あるのが普通です。それらを 1 枚のシートに混ぜると別系統の時刻が入り組んで
読みにくいため、**シートはログストリームごとに分けます**。そのうえで、

- **各ロググループで最も新しいログを持つストリームのシートだけを表示**します
- **それ以外のシートは既定で非表示**（`state="hidden"`）にします
  — シート見出しが何十枚も並ぶと目的のシートを探せなくなるためです

非表示のシートは削除されたわけではなく、**シート見出しを右クリック →「再表示」**
でいつでも表示できます。どのシートがどのストリームかは「ストリーム一覧」シートに
すべて載っています（`サマリ` の「シート数」列で各ロググループのシート枚数も分かります）。

- 「最新」の判定には、**そのシートに出力されたログの最も新しい時刻**（既定では
  ログ本文の時刻）を使います。
- 該当するログが 1 件も無かったロググループにも、指定した事実が分かるように
  空のシートを 1 枚だけ作成します（このシートは表示のままです）。
...
- ロググループが複数のときは、先頭に `NN_`（`サマリ` の No と同じ番号）を付ける
  （別のロググループの似た名前のストリームと取り違えないため）
- 使用できない文字は `_` に置き換える（`2026/07/31/[$LATEST]abc` → `2026_07_31__$LATEST_abc`）
- 31 文字を超える場合は**末尾側**を残す（`i-0abc…` や日付つきストリーム名のように
  先頭が共通しがちで、末尾のほうが識別に役立つため。`NN_` は残します）
- それでも重複する場合は `~2` `~3` … を付けて一意にする
- 該当ログが 1 件も無かったロググループのシートは、ストリーム名の代わりに
  ロググループ名から作ります

**完全なロググループ名・ストリーム名は各シートの 1〜2 行目と「ストリーム一覧」
シートに記録**されるため、シート名が切り詰められても対応関係をたどれます。
...
- ログストリームが非常に多いロググループでは、その数だけシートが作られます
  （非表示のシートも含めてブックのサイズは増えます）。必要に応じて
  `--log-stream-prefix` で対象ストリームを絞り込んでください。
...
| `--excel FILE` | ロググループ × ログストリームごとにシートを分けて Excel 出力。最新ストリーム以外のシートは既定で非表示（`--follow` とは併用不可） |
...
- Excel のシート分割: ストリーム 1 件 / 複数 / ロググループごとに最新ストリームだけが
  表示されること / 他のシートが非表示（`state="hidden"`）で作られ、Excel の「再表示」で
  開けること / 0 件ロググループの空シートが表示のまま作られること /
  ストリーム名が 31 文字超・重複・記号入り（`2026/07/31/[$LATEST]…`）のときのシート名 /
  ロググループが 1 件のときに `NN_` が付かないこと / ストリーム一覧シートの対応表と件数
